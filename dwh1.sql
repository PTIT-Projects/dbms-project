...
CREATE TABLE fact_salary (
	...
	payment_status VARCHAR(20) DEFAULT 'Chưa thanh toán', --trạng thái thanh toán lương
    FOREIGN KEY (employee_sk) REFERENCES dim_employees(employee_sk),
    FOREIGN KEY (date_sk) REFERENCES dim_date(date_sk)
);
...
CREATE TABLE fact_leave_balance (
	...
	date_sk INT,
	granularity VARCHAR(10) DEFAULT 'Năm',  -- 'Năm', 'Quý', 'Tháng'
...
CREATE TABLE fact_work_trips (
    ...
    destination VARCHAR(100), -- Điểm đến của chuyến công tác
    purpose VARCHAR(255), --Mục đích công tác (họp khách hàng, đào tạo, khảo sát...)
    total_cost DECIMAL(10,2) DEFAULT 0, --Tổng chi phí của chuyến công tác 
    status VARCHAR(20),   --'Hoàn thành', 'Đang diễn ra', 'Đã hủy'...
    FOREIGN KEY (employee_sk) REFERENCES dim_employees(employee_sk),
...
CREATE TABLE fact_application (
	...
	position_name VARCHAR(100),
	department_name VARCHAR(100),
    status VARCHAR(20),  -- Trạng thái ứng viên (Đã duyệt, Đã từ chối, Đang xét duyệt, Đã phỏng vấn, Chưa phỏng vấn)
...
CREATE TABLE fact_registrations (
	...
	employee_name VARCHAR(100),
	department_name VARCHAR(100),
    approved_by_name VARCHAR(100), -- Tên người duyệt
    registration_type VARCHAR(50), -- Loại đăng ký (ví dụ: 'Nghỉ phép', 'Làm thêm', 'Công tác', v.v.)
